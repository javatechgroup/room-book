...
public class FacilityFloorIntegrationTest {
	...
	public void testCreateFloorSuccess() throws Exception {
		...
		mockMvc.perform(post("/api/facility/floors").header("Authorization", "Bearer " + adminToken)
				.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request)))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.data.name").value(uniqueName)).andExpect(jsonPath("$.data.floorNumber").value(2))
				.andExpect(jsonPath("$.data.status").value("ACTIVE")).andExpect(jsonPath("$.data.id").exists());
	}
	...
	public void testGetFloorsPaginated() throws Exception {
		mockMvc.perform(
				get("/api/facility/floors?page=1&size=5&status=ALL").header("Authorization", "Bearer " + adminToken))
	...
	public void testGetAllActiveFloors() throws Exception {
		mockMvc.perform(get("/api/facility/floors/all").header("Authorization", "Bearer " + adminToken))
				.andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
	...
	public void testDeleteFloorWithRoomsFails() throws Exception {
		...
		String res = mockMvc
				.perform(post("/api/facility/floors").header("Authorization", "Bearer " + adminToken)
						.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request)))
		...
		mockMvc.perform(delete("/api/facility/floors/" + floorId).header("Authorization", "Bearer " + adminToken))
				.andExpect(status().isConflict()).andExpect(jsonPath("$.code").value("BOOKING_CONFLICT"));
	}
}
